            ...
            {props.data.items.map((project, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Overlay Gradient */}
                  <div 
                    class={`absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-transparent transition-opacity duration-300 ${isActive ? 'opacity-95' : 'opacity-60 group-hover:opacity-95'}`}
                  />

                  {/* Tap Hint Badge (NEW) 
                      Visible by default. Fades out if Active (clicked) OR Hovered.
                  */}
                  <div
                    class={`absolute top-4 right-4 pointer-events-none bg-black/50 backdrop-blur-md border border-white/10 text-white/90 text-xs font-medium px-3 py-1.5 rounded-full flex items-center gap-1 transition-opacity duration-300 ${
